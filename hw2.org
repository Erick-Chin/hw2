...
grammar = nltk.CFG.fromstring("""
    ...
    ADJP -> Adj ADJP | Adj
    VP -> Verb Adv PP | Verb NP | Verb PP
    PP -> Prep NP
    Det -> 'the' | 'a' | 'his'
    Con -> 'and'
    Noun -> 'dog' | 'chair' | 'back' | 'somebody' | 'coffee' | 'we' | 'walk' | 'park' | 'vinny' | 'dogs'
    Adj -> 'cheerful' | 'black' | 'sleepy' | 'yellow' | 'downstairs' | 'three' | 'other' | 'long'
    Verb -> 'slept' | 'stretched' | 'made' | 'had' | 'played'
    Prep -> 'by' | 'to' | 'with'
    Adv -> 'quietly'
""")
...
for sent in sentences:
    trees = rd_parser.parse(sent.lower().replace(".", "").split())
    for tree in trees:
        tree.pretty_print()
#+end_src
