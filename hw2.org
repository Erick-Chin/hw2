...
grammar = nltk.CFG.fromstring("""
    ...
    S -> NP VP Con NP VP
    S -> NP VP Con VP
    NP -> Det ADJP Noun | Det Noun | Noun Adj | Noun | Det Adj Noun PP | ADJP Noun
    ADJP -> Adj ADJP | Adj
    VP -> Verb Adv PP | Verb NP | Verb PP | Verb Adj Adv | Mod Adv Verb Adv
    PP -> Prep NP
    Det -> 'the' | 'a' | 'his'
    Con -> 'and' | 'but'
    Noun -> 'dog' | 'chair' | 'back' | 'somebody' | 'coffee' | 'we' | 'walk' | 'park' | 'vinny' | 'dogs' | 'it'
    Adj -> 'cheerful' | 'black' | 'sleepy' | 'yellow' | 'downstairs' | 'three' | 'other' | 'long' | 'sunny'
    Mod -> 'might'
    Verb -> 'slept' | 'stretched' | 'made' | 'had' | 'played' | 'was' | 'be'
    Prep -> 'by' | 'to' | 'with'
...
for sent in sentences:
    ...
    if len(trees) == 0:
        print(f"No tree for sentence {sent}")
    for tree in trees:
        tree.pretty_print()
#+end_src
...
somebody     downstairs made the     coffee
 ...
 NP   |    |   |    |    |        NP       |    NP    |     |     |         ADJP  |
 |    |    |   |    |    |     ___|___     |    |     |     |     |          |    |
Noun Verb Det Adj  Noun Prep Det     Noun Con  Noun  Verb  Prep  Adj        Adj  Noun
 |    |    |   |    |    |    |       |    |    |     |     |     |          |    |
 we  had   a  long walk  to  the     park and vinny played with three      other dogs
...
Noun Verb  Adj   Adv  Con  Mod  Adv Verb   Adv
 |    |     |     |    |    |    |   |      |
 it  was  sunny today but might not  be  tomorrow
